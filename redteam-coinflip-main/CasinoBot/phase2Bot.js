...
const phase2Response =  async (activeGames) => {
    ...
    for (i = 0; i < activeGames.length; i++){
        ...
        console.log("Processing game for user:",activeGame)
        ...
        const gasEstimate = await casinoContract.methods.casinoCommit(activeGame).estimateGas({ from: myAddress, to: activeGame  });
        
        console.log("Placing bet for user " + activeGame + "....")
        const response = await casinoContract.methods.casinoCommit(activeGame).send({from: myAddress, to: activeGame, gasPrice: gasPrice, gas: gasEstimate })
        console.log("Done!")

...
contract Coinflip_testing {
    ...
    function userSubmitChoice(uint256 choice) external {
        ...
        require(
            msg.sender != casino,
            "This function is intended for users only, not casinos."
        );
        require(
    ...
    function getUserChoiceByKey(address _user) public view returns (uint256) {
        require(msg.sender == casino, "Only the casino may call this function");
        return mapUserChoice[_user];
    }

    function getCasinoSecretChoiceByKey(address _user)
        public
        view
        returns (uint256)
    {
        //require(msg.sender == casino, "Only the casino may call this function");
        return mapCasinoSecretChoice[_user];
    }
    ...
    function getCasinoRandomByKey(address _user) public view returns (uint256) {
        require(msg.sender == casino, "Only the casino may call this function");
        return mapCasinoRandom[_user];
    }

    function getCasinoHashByKey(address _user) public view returns (uint256) {
        require(msg.sender == casino, "Only the casino may call this function");
        return mapCasinoHash[_user];
    }

    function getGameResult(address _user) public view returns (GameResult) {
    ...
    //make game session expire for testing purpose.
    function setTimeforTesting(address _user) public {
        require(msg.sender == casino, "Only the casino may call this function");
        mapGameCreationTime[_user] -= 61;
    }
